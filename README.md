...
- `RULE-*.md` — constraints: what Claude must or must not do (behavior, coding, docs, content, stack, SEO, release, DB design).
...
- **Tier 2 — Contextual, read on signal match:** `RULE-docs.md`, `RULE-content-write.md`, `RULE-stack-akiNuxtCf.md`, `RULE-seo.md`, `RULE-release.md`, `RULE-db-design.md`.
- **Tier 3 — Analytical methods:** `METHOD-flow-audit.md` (refactors, multi-file bugs, fragile flows), `METHOD-deep-think.md` (scope/architecture/value decisions, first-principles and critique-style thinking).

No harness magic: Tier 1 uses the `@path` embed syntax; Tiers 2–3 are trigger instructions telling Claude to Read the file from `~/.aki/claudedoc/` when signals match.
